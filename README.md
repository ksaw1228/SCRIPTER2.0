...
### 1. 유지보수를 위한 문서화, 아키택쳐 패턴 적용 필요

제가 클린 아키텍처를 사용한 이유는 주요한 비즈니스 로직과 애플리케이션에 영향을 주는 외부 요인들을 분리하고, 애플리케이션의 유지보수와 확장성을 높이기 위해서였습니다. 

첫째로, 애플리케이션의 확장성이 떨어지는 문제가 있었습니다. 기존의 구조에서는 새로운 기능이나 외부 시스템을 추가할 때마다 중요한 로직이 영향을 받았기 때문에 애플리케이션 변화에 유연하게 대응하기 어려웠습니다. 
이 문제를 해결하기 위해 클린 아키텍처에서 제안하는 동심원 구조를 활용하여 애플리케이션 내부 영역과 외부 영역을 명확하게 분리하였습니다.

둘째로, 유닛 테스트가 어려운 문제도 있었습니다. 비즈니스 로직이 외부 리소스(데이터베이스, 네트워크 등)에 강하게 의존하면서, 테스트가 복잡하고 시간이 많이 소모되었습니다. 
이 문제를 해결하기 위해 클린 아키텍처에서 강조하는 의존성 규칙을 적용하여, 비즈니스 로직이 외부 의존성을 가지지 않도록 설계하였습니다. 이를 통해 순수한 로직에만 집중할 수 있는 유닛 테스트가 가능해졌습니다.

이러한 방법을 적용하기 위해서 클린 아키텍처를 이용해 구조를 설계하였습니다. 

애플리케이션을 몇 개의 동심원 층으로 나누어 각각의 층(도메인, 유스케이스, 인터페이스 어댑터, 프레임워크와 드라이버)에 역할과 책임을 명확하게 할당하였습니다.
각 층에서는 하위 층에만 의존하게 될 수 있도록 의존성 규칙을 준수하였습니다.
...
---
### 6. Repository 통합을 통한 유지,보수성 향상 필요 [typeORM 0.3]

...
첫째로, 애플리케이션의 확장성이 떨어지는 문제가 있었습니다. 기존의 구조에서는 새로운 기능이나 외부 시스템을 추가할 때마다 중요한 로직이 영향을 받았기 때문에 애플리케이션 변화에 유연하게 대응하기 어려웠습니다. 이 문제를 해결하기 위해 클린 아키텍처에서 제안하는 동심원 구조를 활용하여 애플리케이션 내부 영역과 외부 영역을 명확하게 분리하였습니다.

둘째로, 유닛 테스트가 어려운 문제도 있었습니다. 비즈니스 로직이 외부 리소스(데이터베이스, 네트워크 등)에 강하게 의존하면, 테스트가 복잡하고 시간이 많이 소모되기 마련입니다. 이 문제를 해결하기 위해 클린 아키텍처에서 강조하는 의존성 규칙을 적용하여, 비즈니스 로직이 외부 의존성을 가지지 않도록 설계하였습니다. 이를 통해 순수한 로직에만 집중할 수 있는 유닛 테스트가 가능해졌습니다.

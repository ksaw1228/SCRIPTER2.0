...
NestJS의 클린 아키텍처와 싱글톤 패턴의 가이드를 따라가며 프로젝트를 구현하였습니다. 클린 아키텍처는 의존성이 외부에서 내부로만 존재하고, 비즈니스 로직(엔티티)이 외부에 대해 전혀 알지 못해도 되는 구조입니다.

클린 아키텍처 패턴은 각 영역의 책임을 분명하게 하여 비즈니스 로직과 외부 사항 간의 결합도를 최소화합니다. NestJS 프레임워크를 사용하면서 이를 실현하기 위해 각 레이어의 구성요소를 분리하여 의존성 주입과 싱글톤 패턴을 적용하였습니다.

이런 방식으로 프로젝트의 구조가 더욱 명확해지고, 변화에 유연하게 대응할 수 있는 설계가 되었습니다. 결과적으로 비즈니스 로직을 효과적으로 독립시켜 확장성을 높여주고, 외부와의 상호작용을 쉽게 관리할 수 있게 되었습니다.
...
기존에 존재하던 프로젝트를 처음부터 NestJS로 전환하는 작업이었고, 이 말은 이미 바람직한 Green Test 결과값을 알고 있다는 것이기 때문에
비교적 빠른 속도로 테스트 코드 작성이 가능하였습니다. 이로 인해 빠르면서 안정적인 리팩토링을 할 수 있었습니다.
